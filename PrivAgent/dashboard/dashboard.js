/**
 * PrivAgent :: Dashboard
 * All Overview/Analytics/Voice Agent numbers are computed from
 * chrome.storage.local history + settings — never randomly generated.
 * Demo Mode swaps in a clearly-labelled, separate mock dataset and never
 * reads or writes real history.
 */
...
(function () {
  ...
  const TABS = ['overview', 'protection', 'activity', 'voice', 'analytics', 'settings'];
  ...
  function buildDemoHistory() {
    ...
    const sample = [
      ...
      ['voice_command', 'OPEN_WEBSITE', 'executed', 'LOW'],
      ['pii_detected', 'Credit/debit card', 'protected', 'HIGH'],
      ['setting_changed', 'protection', 'enabled', 'LOW'],
      ['voice_command', 'SEARCH_GOOGLE', 'executed', 'LOW']
    ];
    for (let i = 0; i < 24; i++) {
      const s = sample[i % sample.length];
      t -= (2 + Math.random() * 10) * 3600 * 1000;
      events.push({ id: `demo_${i}`, timestamp: t, domain: domains[i % domains.length], eventType: s[0], dataType: s[1], action: s[2], riskLevel: s[3] });
    }
    return events;
  }
  ...
  // ---------------- Tab routing (hash-based so the popup/voice agent can deep-link) ----------------
  ...
  async function renderOverview() {
    ...
    el('ov-req-blocked').textContent = '0';
  ...
  async function updateSetting(partial) { await self.PrivAgentStorage.setSettings(partial); }

  async function broadcastSettings() {
    const settings = await self.PrivAgentStorage.getSettings();
    chrome.tabs.query({}, (tabs) => { tabs.forEach((t) => chrome.tabs.sendMessage(t.id, { type: 'SETTINGS_UPDATED', settings }).catch(() => {})); });
  }
  ...
  function applyHistoryFilters() {
    ...
    renderHistoryList('historyList', filtered);
    el('h-empty').hidden = filtered.length > 0;
  }

  function renderHistoryList(listId, entries) {
  ...
  function eventTitle(h) {
    switch (h.eventType) {
      ...
      case 'voice_command': return `Voice command: ${friendlyIntent(h.dataType)}`;
  ...
  function friendlyIntent(intent) {
    const map = {
      OPEN_WEBSITE: 'Open website', SEARCH_YOUTUBE: 'Search YouTube', SEARCH_GOOGLE: 'Search Google',
      TAB_BACK: 'Go back', TAB_FORWARD: 'Go forward', RELOAD: 'Reload page', NEW_TAB: 'New tab',
      CLOSE_TAB: 'Close tab', SCROLL_DOWN: 'Scroll down', SCROLL_UP: 'Scroll up',
      SCROLL_BOTTOM: 'Scroll to bottom', SCROLL_TOP: 'Scroll to top', OPEN_DASHBOARD: 'Open dashboard',
      OPEN_SETTINGS: 'Open settings', SHOW_ACTIVITY: 'Show activity', ENABLE_PROTECTION: 'Enable protection',
      DISABLE_PROTECTION: 'Disable protection', not_supported: 'Not understood'
    };
  ...
  el('h-clear').addEventListener('click', () => {
    showConfirm('Are you sure you want to clear your privacy history?', async () => {
      ...
      await renderActivity(); await renderOverview(); await renderAnalytics(); await renderVoice();
    });
  });
  ...
  async function renderVoice() {
    const { settings, history, demo } = await loadData();
    el('demoBanner').hidden = !demo;
    const voiceEvents = history.filter((h) => h.eventType === 'voice_command').sort((a, b) => b.timestamp - a.timestamp);
    el('v-executed').textContent = String(voiceEvents.filter((h) => h.action === 'executed').length);
    el('v-failed').textContent = String(voiceEvents.filter((h) => h.action === 'not_supported').length);
    el('v-enabled').textContent = settings.voiceCommandsEnabled ? 'Enabled' : 'Disabled';
    renderHistoryList('voiceList', voiceEvents.slice(0, 30));
    el('v-empty').hidden = voiceEvents.length > 0;
  }
  ...
  async function renderAnalytics() {
    ...
    Object.entries(byType).sort((a, b) => b[1] - a[1]).forEach(([type, count]) => {
      ...
      row.innerHTML = `<span>${escapeHtml(type)}</span><div class="bar-track"><div class="bar-fill" style="width:${(count / maxCount) * 100}%"></div></div><span>${count}</span>`;
      byTypeEl.appendChild(row);
    });

    const days = [];
    const now = new Date();
    for (let i = 13; i >= 0; i--) { const d = new Date(now); d.setDate(now.getDate() - i); d.setHours(0, 0, 0, 0); days.push(d); }
    ...
    days.forEach((d, i) => {
      ...
      col.className = 'col';
      col.innerHTML = `<div class="bar" style="height:${(dayCounts[i] / maxDay) * 70 + 2}px" title="${dayCounts[i]} events"></div><span class="day-label">${d.getDate()}</span>`;
    ...
    days.forEach((d) => {
      ...
      const col = document.createElement('div');
      col.className = 'col';
      col.innerHTML = `<div class="bar" style="height:${(score / 100) * 70 + 2}px" title="Score ${score}"></div><span class="day-label">${d.getDate()}</span>`;
  ...
  el('s-demoMode').addEventListener('change', async (e) => { await updateSetting({ demoMode: e.target.value === 'true' }); await renderAll(); });
  ...
  el('s-clearAll').addEventListener('click', () => {
    showConfirm('This erases ALL PrivAgent settings and history and cannot be undone. Continue?', async () => {
      await self.PrivAgentStorage.clearAllData(); await broadcastSettings(); await renderAll();
    });
  });
  el('s-resetSettings').addEventListener('click', () => {
    showConfirm('Reset all settings to their defaults? History will be kept.', async () => {
      await self.PrivAgentStorage.resetSettings(); await broadcastSettings(); await renderAll();
    });
  });
  ...
  async function renderAll() {
    await renderOverview(); await renderProtection(); await renderActivity();
    await renderVoice(); await renderAnalytics(); await renderSettings();
  }
